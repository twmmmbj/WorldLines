...
不要自己编造骰子数字。每次命中/先攻/对抗检定都调用 `Skill(skill="dice")` 的确定性 CoC 7e `roll_check(skill_name, target)`（target = 对应技能/命中值 0–100），消费它返回的结构化 `DiceResult`（`roll` / `outcome` / `passed` / `margin`）来决定命中、先攻顺序与后果。

每个检定的受理行用 `format_dice_line` 的固定结构行，不写成散文数字：

```
[<检定名>: <roll> <op> <target>] → <结果>
```

`<op>` 通过为 `<=`、未过为 `>`；结果等级：`critical`(01) / `extreme`(≤target÷5) / `hard`(≤target÷2) / `success`(≤target) / `fail`(>target) / `fumble`(96–100)。

...
**掷骰：不要自己编造骰子数字。** 调用 `Skill(skill="dice")` 走确定性 CoC 7e 检定（`roll_check(skill_name, target)`，target = 角色技能值 0–100）。它返回结构化 `DiceResult`（`roll` / `outcome` / `passed` / `margin`）。你消费这个结果——不自创点数、不自定成功等级。

**等级（由 roll 对 target 推导，roll-under）：**
- `critical` 01 大成功 ／ `extreme` ≤target÷5 ／ `hard` ≤target÷2 ／ `success` ≤target ／ `fail` >target ／ `fumble` 96–100 大失败

输出受理行用固定结构行 `[<名称>: <roll> <op> <target>] → <结果>`（来自 `format_dice_line`），不要写成散文数字。

...
> **本地运行**(TUI / CLI)。multi-agent 暂未上 hosted play —— 在线只支持 `fast` + `orch`。clone 仓库后用 `neonrp play --project examples/multi-agent/<world>` 运行(`neonrp play` 是专用的村庄运行器)。**新手?从 Stoneford · Elena 开始** —— 村庄模型最易上手的尝鲜。
...
neonrp play --project examples/multi-agent/stoneford-elena   # 2 个灵魂 —— Elena & Rowan  ← 从这开始
neonrp play --project examples/multi-agent/kagura-island     # 7 个灵魂 —— 和风悬疑

...
> **로컬 실행**(TUI / CLI). multi-agent는 아직 호스티드 플레이 미지원 —— 온라인은 `fast` + `orch`만. 저장소를 clone 후 `neonrp play --project examples/multi-agent/<world>` 로 실행(`neonrp play` 는 전용 마을 러너). **처음이라면 Stoneford · Elena부터** — 마을 모델을 가장 쉽게 맛보는 길.
...
neonrp play --project examples/multi-agent/stoneford-elena   # 소울 2명 — Elena & Rowan  ← 여기서 시작
neonrp play --project examples/multi-agent/kagura-island     # 소울 7명 — 일본풍 미스터리

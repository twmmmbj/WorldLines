...
あなたのすべての行動を覚えている世界に足を踏み入れよう。灰霧の北の港で包囲命令を下す。あなたの言葉を本当に覚えている記憶喪失の癒し手と向かい合って座る。数分で世界を作り、そのルール、テンポ、NPC を設定するか、公開カタログから選んでそのまま飛び込む。これはリアルタイムのマルチエージェント社会：多くの AI ソウルとあなたが、ひとつの世界で、共に形づくる。

WorldLines は**ロールプレイのためのマルチエージェント・シミュレーションエンジン**です：世界そのものが、独立した agent たちの社会。world-agent が世界を保有し、**soul-agent** の一団を編成します —— どのキャラクターも、固有の心・記憶・思惑を持つそれ自身の agent であり、対話データの一行ではありません。あなたは分身端末で参加します。これはチャットボットでも、スクリプトゲームでもありません。
...
ダブルクリックでインストール——初回起動時にエンジンを自動セットアップし、
ローカル Web 製品をフルで開きます。ターミナル不要。
...
アプリは未署名です。macOS では「壊れている」と表示されます——アプリケーションに
コピー後、ターミナルで一度 `xattr -cr /Applications/WorldLines.app` を実行して
ください。Windows：SmartScreen → 詳細情報 → 実行。Linux は下記のターミナル
インストールを。
...
<p align="center">
  <img src="./assets/multi-agent-play.png" alt="WorldLines — マルチエージェントプレイ、シーンに7つの魂" width="720" />
</p>
<p align="center"><em>マルチエージェントプレイ（神楽島）—— シーンに 7 つの魂；このターンは 3 体が並行で思考し、残りは背後で生き続ける。</em></p>

---

## サンプルワールド

WorldLines は**それ自体が multi-agent simulation（マルチエージェント・シミュレーション）**です：world-agent が**独立した魂（souls）**の一団を包みます —— それぞれが固有の心・記憶・秘密・目的を持つ character-agent（目印は `souls/` フォルダ）。これがエンジンの本体であり、すべてのワールドが向かう先です。

> 軽量モードはより単純な単線シーン向けに残してあります —— `orch` は world-agent がドメイン agent を束ね、NPC はデータ;`fast` は単一ボイスの agent —— ですが WorldLines の核心は下のマルチエージェント社会です。

### 👥 マルチエージェント社会 —— 一つのワールドに独立した魂
...
### ⛩ その他：軽量な `orch` / `fast` ワールド

これらは world-agent がデータ駆動の NPC を演じます —— per-soul agent はありません。よりタイトな単線シーン向けで、上のフルなマルチエージェント社会への入り口にもなります。

**Stoneford** —— 灰霧の北の河港。クラシックファンタジー TRPG · d20 ダイス · world-agent が town / dungeon / combat / story agent にルーティング。**[オンラインで遊ぶ →](https://hub.worldlines.gg/play/worlds/stoneford)** · **[ソース & ドキュメント →](./examples/orch/stoneford)**
...
```
world-agent          — 正規のワールドを保有：状態 · ルーティング · 語り · アーカイブ
  └─ soul-agents     — キャラごとに1体、それぞれ自分の心：
                        人格 · 記憶（長期+短期）· 秘密 · 目的 · 内なる声
                        自ら知覚し、決断し、行動する
  （軽量モード）      — orch はドメイン agent（town / dungeon / combat / story）を追加、
                        NPC はデータ；fast は単一ボイスの agent
```
...
| **v0.3.0** — Desktop (2026-07) | ✓ Web/TUI 統合 · Web Launcher · ホワイトボックス創作フロー · セルフホスト Web 版 |
| **v1.0** — Protocol | ○ 安定した WORLD/SOUL プロトコル · 常駐ワールド |

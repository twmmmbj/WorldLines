...
> **ローカル実行**（TUI / CLI）。multi-agent はまだホストプレイ非対応 —— オンラインは `fast` + `orch` のみ。リポジトリを clone して `neonrp play --project examples/multi-agent/<world>` で実行（`neonrp play` は村専用ランナー）。**初めてですか？Stoneford · Elena から始めよう** —— 村モデルの一番やさしい入り口です。
...
neonrp play --project examples/multi-agent/stoneford-elena   # 2 体のソウル —— Elena & Rowan  ← ここから
neonrp play --project examples/multi-agent/kagura-island     # 7 体のソウル —— 和風ミステリー
